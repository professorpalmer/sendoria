...
def load_config():
    ...
    try:
        ...
        for line in lines:
            ...
            # Parse BOT_TOKEN=value
            if line.startswith('BOT_TOKEN='):
                BOT_TOKEN = line.split('=', 1)[1].strip()
                continue
            
            # Parse CHANNEL_Type=id
            if line.startswith('CHANNEL_'):
                parts = line.split('=', 1)
                if len(parts) == 2:
                    channel_key = parts[0].strip()
                    channel_id = parts[1].strip()
                    
                    # Extract chat type from CHANNEL_Type format
                    if channel_key.startswith('CHANNEL_'):
                        chat_type = channel_key[8:]  # Remove 'CHANNEL_' prefix
                        try:
                            CHANNEL_MAP[chat_type] = int(channel_id)
                        except ValueError:
                            print(f"Warning: Invalid channel ID for {chat_type}: {channel_id}")
    ...
    if message.channel.id in reverse_channel_map:
        ...
        try:
            ...
            if chat_type == 'Tell' and (content.startswith('/tell ') or content.startswith('/t ')):
                ...
                # For Linkshell1 and Linkshell2, include Discord username
                if chat_type in ['Linkshell1', 'Linkshell2']:
                    ...
                    # For other chat types, don't include username
                    with open(RESPONSE_FILE, 'a', encoding='utf-8') as f:
